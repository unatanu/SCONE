function init( model, par )
	...
	subject_assist = model:find_body( "calcn_r" )  -- 人工筋の駆動タイミングを管理するための身体部位（左足のかかと接地で駆動）
	joint_back = model:find_joint( "back" )
	joint_hip = model:find_joint( "hip_l" )
	joint_knee = model:find_joint( "knee_l" )
...
function update( model )
	...
	if step_count_r >= assist_start_gaitcycle then
...
function store_data( frame )
	frame:set_value( "moment_arm_l", moment_arm )
	frame:set_value( "assist_moment_l", assist_moment )
	
	frame:set_value( "F_spring_l", F_spring )
	frame:set_value( "F_damper_l", F_damper )
	frame:set_value( "T_ratio_l", T_ratio )
	
	frame:set_value( "command_l", command )
	
	frame:set_value( "step_count_r", step_count_left )
end
